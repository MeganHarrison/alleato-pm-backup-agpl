...
- [x] Cycles and Modules fail closed unless the mutation-capable server-only
...
- [x] No database, provider, authentication, permission, or deployment state is
  changed.
...
- Recovery path: use one of the six supported surface slugs or integrate the
  missing typed feature export at this dispatcher boundary. Cycles and Modules
  may be mounted only with the mutation-capable server-only
  `PLANE_SCHEDULE_ADAPTER_MUTATION_PREVIEW=true`; enabling it permits
  create/update/delete schedule writes. The old flag and every `NEXT_PUBLIC`
...
- Cycles and Modules remain production-disabled until a dedicated,
  discriminator-safe data migration is designed, applied, and verified.
