        ...
        try:
            ...
            try:
                ...
                _record_sync_run_safe(
                    ...
                    items_seen=result.items_synced + result.items_failed,
                    items_synced=result.items_synced,
                    items_failed=result.items_failed,
                    error_message=error,
                    metadata={"retry_scheduled": result.retry_required},
                )
                summary["sharepoint"] += result.items_synced
                summary["sharepoint_failed"] += result.items_failed
                if result.retry_required:
                    summary["errors"].append(f"SharePoint recovery scheduled for {resource_name}: {error}")
            except Exception as exc:
                ...
                _save_sync_state(supabase, "sharepoint_file", resource_id, resource_name, token or "", 0, "error", str(exc))
                _record_sync_run_safe(
                    supabase,
                    source="sharepoint_file",
                    resource_id=resource_id,
