...
tags: [ai-assistant, ask-alleato, eve, skills, rag, intelligence, index, overview]
featured: true
client_visible: true
ai_visible: true
order: 599
...
| Feature | Where It Lives | What It Does |
|---------|---------------|--------------|
| **Financial analysis skill** | Eve | Analyzes margin, cash position, change order exposure, AP/AR, and billing |
...
| **Risk review skill** | Eve | Risk-focused analysis: overdue RFIs, submittal flags, budget variance, schedule slippage |
...
### Find and understand actions

Eve can explain a workflow and link you to the correct application page. Eve's
current production bridge is read-only, so it cannot create or update RFIs,
change events, change orders, tasks, commitments, daily reports, knowledge
records, email drafts, calendar invitations, or Teams messages from chat.

Typed implementations and preview/approval UI exist for future governed action
work, but they are not exposed to Eve today.

→ See [AI Assistant Actions and Current Limits](/help/articles/ai-assistant-actions)
...
| **Operations review skill** | Eve | Field operations: schedule status, daily log summaries, open punch items, staffing |
| **People capacity skill** | Eve | Team capacity, over-allocation, directory, role assignments |

→ See [My Work](/help/articles/my-work) and [Schedule](/help/articles/schedule)

---

## Eve's Six Domain Skills

Every question goes through Eve. Eve loads the relevant domain skills and can
combine them for cross-domain questions.

| Skill | Domain | Sample Questions |
|---------|--------|-----------------|
| **Financial analysis** | Finances, margin, cash, ERP | "What's our cash position?" / "Where are we losing margin?" |
| **Operations review** | Field ops, schedule, daily logs | "What tasks are running late on Vermillion Rise?" |
| **Risk review** | Risk, exposure, overdue items | "What are our top risks across all projects?" |
| **People capacity** | Team, staffing, capacity | "Who is over-allocated next month?" |
| **Business development** | Pipeline, market intelligence, pursuits | "Research tilt-up warehouse costs in our market." |
| **Marketing strategy** | Positioning, campaigns, content planning | "Build a source-backed case-study outline for this project." |

The business-development and marketing skills can use web capabilities only
when those capabilities are enabled for the Eve deployment.
...
- **Create, update, delete, or send from chat** — Eve's current bridge is read-only
- **Approve, sign, or pay** on your behalf
- **Access records outside your permissions** — the AI is scoped to projects you can access
...
| Article | Description |
|---------|-------------|
| [AI Assistant Overview](/help/articles/ai-assistant-overview) | How Eve works, its six domain skills, and conversation history |
| [AI Assistant Actions and Current Limits](/help/articles/ai-assistant-actions) | Current read-only boundary and future governed action requirements |
...
| [Ask the AI About Finances](/help/articles/ai-financial-advisor) | Financial analysis skill, budget, margin, cash flow, Acumatica |
