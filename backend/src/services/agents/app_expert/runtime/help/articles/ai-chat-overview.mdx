...
Open the AI Assistant at **`/ai`** from the main navigation.

There are two entry surfaces:

- **Full chat page** (`/ai`) — the dedicated Eve workspace with conversation history and assistant controls.
...
- **Artifacts** — workspace records can open in a side panel so you can read or
  copy them without losing the chat. Eve can read available artifacts but cannot
  create or edit them through the current read-only bridge.

### Side panels (right)

Opened from the toolbar at the top of the chat:

- **Turn evidence** — durable turn events and source receipts show which
  request-scoped tools and evidence contributed to an answer.
...
It's also fine to ask it about itself — “what tools do you have?”, “what data
sources can you see?”, or “which skill applies?” — and it can use these help
articles to answer.
...
- **Across conversations** — the assistant can retain approved facts about you
  and your projects. Review the available controls under **`/ai`**, or ask Eve
  what it remembers.
...
The assistant is one Eve identity with six skills: financial analysis,
operations review, risk review, people capacity, business development, and
marketing strategy. Eve can load one or several skills without handing the
conversation to a separate frontend agent.

You can ask Eve to use a specific lens, such as “review this through the
...
Not yet from the UI. If you need a thread captured, copy the messages directly
or ask Eve for a text summary, then save it through the normal workspace UI.
Eve cannot create the artifact through the current read-only bridge.

**What model is the assistant using?**
Model selection belongs to the Eve deployment and provider configuration, not
to separate frontend agents. See [AI Models & Providers](/architecture/ai-rag).
...
- [AI Architecture](/architecture/ai-rag) — Eve, skills, retrieval, and ingestion ownership
