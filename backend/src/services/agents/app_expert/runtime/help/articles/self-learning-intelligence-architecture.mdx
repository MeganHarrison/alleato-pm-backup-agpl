...
### Attribution And Signal Review Ledgers
...
The former per-document compiler queues are retained only as historical audit
records. They are not ingestion stages and must not receive new work.
...
7. Ingestion ends after durable source storage, attribution, and embedding.
8. The governed Daily Executive Brief enumerates the complete source set on its schedule.
9. Canonical packet consumers project supported findings, tasks, reports, and current state.
...
1. User message enters the Eve client and authenticated
...
1. A task proposal originates from extraction or a supported application
   workflow. Eve's current production bridge is read-only and does not create
   the task from chat.
