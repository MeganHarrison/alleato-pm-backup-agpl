...
const AISDK_FEATURES: Array<{
  ...
  { feature: "Eve event stream", usage: "The main Alleato Assistant streams durable messages, tool calls, approvals, and resumable session events through Eve.", where: "agents/alleato-assistant, use-alleato-eve-chat.ts", badge: "Core" },
  { feature: "generateText", usage: "Meeting prep generation, Procore RAG Q&A, and other isolated non-Assistant generation.", where: "meeting prep, procore-docs/ask, fallback-chain", badge: "Core" },
  ...
  { feature: "Output.object() / Output.array()", usage: "Structured output from generateText without switching to generateObject — used for action briefs and suggestion arrays.", where: "structured-output.ts, request-suggestions.ts", badge: "Tools" },
  { feature: "Eve skills and tools", usage: "One Assistant identity selects executive skills and authenticated Alleato tools at runtime.", where: "agents/alleato-assistant/agent", badge: "Agents" },
  ...
  { feature: "useEveAgent", usage: "Client-side durable Assistant sessions, messages, tool state, approvals, and reconnect behavior.", where: "use-alleato-eve-chat, useAskAlleatoChat", badge: "React" },
  { feature: "Dynamic Eve auth", usage: "Every Eve request receives the current Supabase bearer token, which the agent validates before exposing authenticated tools.", where: "use-alleato-eve-chat.ts, agent/channels/eve.ts", badge: "React" },
  { feature: "Eve input responses", usage: "Human-in-the-loop approvals return through Eve's durable input-response protocol.", where: "use-alleato-eve-chat.ts", badge: "HITL" },
  { feature: "Eve message and tool parts", usage: "Eve event parts are adapted into the shared ChatArea renderer without a second Assistant runtime.", where: "use-alleato-eve-chat.ts → chat-area.tsx", badge: "Generative UI" },
