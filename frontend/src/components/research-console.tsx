...
export function ResearchConsole() {
  ...
  const startRun = useCallback(async () => {
    ...
    try {
      const body = await apiFetch<{ runId?: string; error?: string }>("/api/research", {
        method: "POST",
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(async () => {
      try {
        const nextStatus = await apiFetch<RunStatus>(`/api/run/${runId}`);
        ...
        <section className="pb-20 md:pb-24">
          <div className="mb-5 flex items-baseline gap-4">
            ...
            <SectionRuleHeading
              ...
              label={
                <span className="text-[22px] font-semibold normal-case tracking-normal text-[#f5f5f5] md:text-[24px]">
                  Research question
                </span>
              }
            />
              ...
              {report.sections.map((section) => (
                <section key={section.heading} className="max-w-[980px]">
                  <SectionRuleHeading
                    ...
                    className="mb-0 gap-0 pb-0"
                    label={
                      <span className="text-[19px] font-semibold normal-case tracking-normal text-[#f1f1f1]">
                        {section.heading}
                      </span>
                    }
                  />
