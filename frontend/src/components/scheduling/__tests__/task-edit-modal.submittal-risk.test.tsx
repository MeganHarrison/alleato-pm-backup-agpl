...
jest.mock("@/lib/api-client", () => ({
  apiFetch: jest.fn((url: string) => Promise.resolve(
...
it("makes a failed submittal link visible instead of leaving an unhandled action", async () => {
  ...
  expect(await screen.findByText("You do not have permission to link this submittal.")).toHaveAttribute("role", "alert");
});

it("does not report an empty, clear submittal state when the authoritative risk read failed", async () => {
