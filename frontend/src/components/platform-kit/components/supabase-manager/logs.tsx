...
import { Check, ChevronsUpDown, Logs } from 'lucide-react'
...
import { useGetLogs } from '@/components/platform-kit/hooks/use-logs'
...
export function LogsManager({ projectRef }: { projectRef: string }) {
  ...
  const { data: logs, isLoading, error, refetch } = useGetLogs(projectRef, { sql })
    ...
    <>
      ...
      {(error || (logs && logs.error)) && (
        <div className="mx-6 lg:mx-8 mt-8">
          <ErrorState
            title="Couldn't fetch logs"
            description={
              error?.message ||
              (typeof logs?.error === 'object' ? logs.error?.message : undefined) ||
              undefined
            }
      ...
      {!error && logs && logs.result && logs.result.length === 0 && (
        <div className="mt-8 mx-8">
          <EmptyState
            icon={<Logs />}
            title="No logs found"
            description="Logs will appear here when your application generates activity."
          />
        </div>
      )}
    </>
