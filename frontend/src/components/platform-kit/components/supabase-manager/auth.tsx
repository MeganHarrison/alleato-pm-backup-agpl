...
import {
  useGetAuthConfig,
  useUpdateAuthConfig,
} from '@/components/platform-kit/hooks/use-auth'
...
function ProviderSettingsView({
  ...
  projectRef: string
  // Zod v4: `.refine()`/`.superRefine()` on an object now return a ZodObject
  // (the old ZodEffects wrapper was removed), so every provider schema here has
  // `.shape` directly — no unwrapping needed.
    ...
    return schemaKeys.reduce(
      (acc, key) => {
        if (Object.prototype.hasOwnProperty.call(authConfigData, key)) {
          acc[key] = (authConfigData as Record<string, unknown>)[key]
        }
        return acc
      },
      {} as Record<string, any>
    ...
    return (
      <div className="mx-6 lg:mx-8 mt-8">
        <ErrorState
          ...
          error={errorLoadingConfig}
          onRetry={() => void refetchAuthConfig()}
        />
      </div>
    )
  }
