...
import { Key, Minus, PlusIcon } from 'lucide-react'
...
import {
  ...
  useDeleteSecrets,
  useGetSecrets,
} from '@/components/platform-kit/hooks/use-secrets'

export function SecretsManager({ projectRef }: { projectRef: string }) {
  const { data: secrets, isLoading, error, refetch } = useGetSecrets(projectRef)
      ...
      <div>
        ...
        {secrets && secrets.length > 0 ? (
          ...
          <EmptyState
            ...
            description="No secrets found for this project."
          />
        )}
      </div>
    </div>
