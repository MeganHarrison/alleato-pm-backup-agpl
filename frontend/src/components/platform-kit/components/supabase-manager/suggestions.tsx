...
import { useGetSuggestions } from '@/components/platform-kit/hooks/use-suggestions'

export function SuggestionsManager({ projectRef }: { projectRef: string }) {
  const { data: suggestions, isLoading, error, refetch } = useGetSuggestions(projectRef)
        ...
        <div>
          {sortedSuggestions.length > 0 ? (
            ...
            <EmptyState
              ...
              description="Your project looks good! No suggestions at this time."
            />
          )}
        </div>
      )}
