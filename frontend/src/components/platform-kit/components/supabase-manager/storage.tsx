...
import { useGetBuckets } from '@/components/platform-kit/hooks/use-storage'

export function StorageManager({ projectRef }: { projectRef: string }) {
  const { data: buckets, isLoading, isError, error, refetch } = useGetBuckets(projectRef)
  ...
  return (
    <div className="p-6 pt-4 lg:p-8 lg:pt-8">
      ...
      {isError && (
        <ErrorState
          title="Couldn't load storage buckets"
      ...
      {buckets && buckets.length > 0 ? (
        ...
        <EmptyState
          ...
          description="A bucket is a container used to store and protect files in your app."
        />
      ) : null}
    </div>
  )
