...
import { Table, Wand } from 'lucide-react'
...
import { SqlEditor } from '@/components/platform-kit/components/sql-editor'
import { useSheetNavigation } from '@/components/platform-kit/contexts/SheetNavigationContext'
import { useRunQuery } from '@/components/platform-kit/hooks/use-run-query'
import { useListTables } from '@/components/platform-kit/hooks/use-tables'
...
export function DatabaseManager({ projectRef }: { projectRef: string }) {
  ...
  const { data: tables, isLoading, isError, error, refetch } = useListTables(projectRef, [
    'public',
  ])
  ...
  return (
    <div className="p-6 pt-4 lg:p-8 lg:pt-8">
      ...
      {/*
        Error and empty are mutually exclusive, and "empty" requires data we
        actually received. A failed request has no rows either, and rendering the
        empty state for it reads as "this database has no tables" — which is how
        a 403 from the Management API disguised itself as an empty PM App.
      */}
      {isError ? (
        <ErrorState
        ...
        <EmptyState
          ...
          description="Create tables to store and organize your data."
        />
      ) : null}
    </div>
  )
