...
// These helpers duck-type into the Zod internals to support BOTH the old
// (typeName) and new v4 (type) shapes. In v4 `ZodObject.shape` values are typed
// as the internal `$ZodType`, so we accept `any` here rather than a public
// Zod type — the runtime checks below do the real work.
const getZodDef = (schema: any): Record<string, any> =>
  (schema as any)._def || (schema as any).def

// Read the option list off a ZodEnum across Zod versions: v3 exposes
// `_def.values` (array); v4 exposes `_def.entries` (object) and `.options`
// (array) on the instance. Without this, v4 enum fields render an empty <Select>.
const getEnumValues = (enumSchema: any): string[] => {
  const def = getZodDef(enumSchema) || {}
  if (Array.isArray(def.values)) return def.values
  if (def.entries && typeof def.entries === 'object') return Object.values(def.entries)
  if (Array.isArray(enumSchema?.options)) return enumSchema.options
  return []
}
...
const unwrapZodType = (fieldSchema: any): any => {
  ...
  while (
    ...
    // Zod v4: `.preprocess()` and `.transform()` are modeled as a ZodPipe
    // instead of the v3 ZodEffects wrapper.
    getTypeName(getZodDef(currentSchema)) === 'pipe'
  ) {
    ...
    if (typeName === 'ZodEffects' || typeName === 'effects') {
      ...
      // A ZodPipe has an `in` and `out` side; the transform function lives on
      // one of them. Descend into the side that is NOT the transform so we
      // reach the real underlying schema (works for both `.transform()` where
      // the schema is `in`, and `.preprocess()` where it is `out`).
      const inIsTransform = getTypeName(getZodDef(def.in)) === 'transform'
      currentSchema = inIsTransform ? def.out : def.in
    } else {
...
export function DynamicForm<T extends z.ZodRawShape = z.ZodRawShape>({
    ...
    (acc, key) => {
      ...
      switch (getTypeName(getZodDef(baseType))) {
        ...
        case 'enum':
          ...
          const enumValues = getEnumValues(baseType)
  ...
  const form = useForm<z.infer<typeof schema>>({
    // Zod v4 distinguishes a schema's input vs output type when it contains
    // .transform()/.preprocess()/.default(); zodResolver is keyed by the input
    // type while the form is keyed by the output type, so the Resolver generic
    // no longer unifies. The runtime behavior is unchanged — cast the boundary.
      ...
      const processedInitialValues = schemaKeys.reduce(
        (acc, key) => {
          ...
          const value = initialValues.hasOwnProperty(key)
            ? (initialValues as Record<string, any>)[key]
            : undefined
          ...
          if (fieldTypeName === 'ZodBoolean' || fieldTypeName === 'boolean') {
            ...
            const enumValues = getEnumValues(baseFieldType)
            acc[key] =
      ...
      <FormField
        ...
        render={({ field }) => {
          ...
          switch (typeName) {
            ...
            case 'ZodEnum':
            case 'enum':
              const options = getEnumValues(baseType)
