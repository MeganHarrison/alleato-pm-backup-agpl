...
import { CloseIcon } from "@/components/ui-library/expandable-card-demo-standard";
  ...
  return (
    <>
            ...
            >
              ...
              <div>
                <div className="flex justify-between items-start p-4">
                  <div className="">
                    <motion.h3
                      layoutId={`grid-title-${active.title}-${id}`}
                      className="font-bold text-neutral-700 dark:text-neutral-200"
                    >
                      {active.title}
                    </motion.h3>
                    <motion.p
                      layoutId={`grid-description-${active.description}-${id}`}
                  ...
                  <motion.a
                    layoutId={`grid-button-${active.title}-${id}`}
                    href={active.ctaLink}
                    target="_blank"
      ...
      <ul className="max-w-2xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 items-start gap-4">
        {cards.map((card) => (
          <motion.div
            layoutId={`grid-card-${card.title}-${id}`}
            key={`grid-card-${card.title}-${id}`}
            onClick={() => setActive(card)}
            className="p-4 flex flex-col hover:bg-neutral-50 dark:hover:bg-neutral-800 rounded-xl cursor-pointer"
          >
            <div className="flex gap-4 flex-col w-full">
              <motion.div layoutId={`grid-image-${card.title}-${id}`}>
                <img
                  width={200}
                  height={200}
                  src={card.src}
              ...
              <div className="flex justify-center items-center flex-col">
                <motion.h3
                  layoutId={`grid-title-${card.title}-${id}`}
                  className="font-medium text-neutral-800 dark:text-neutral-200 text-center md:text-left"
                >
                  {card.title}
                </motion.h3>
                <motion.p
                  layoutId={`grid-description-${card.description}-${id}`}
