...
describe("chat surface contract", () => {
  it("treats legacy conversations as general assistant conversations", () => {
    ...
    expect(conversationMatchesSurface({}, "asrs")).toBe(false);
  });

  it("keeps explicitly namespaced ASRS conversations out of general chat", () => {
    const metadata = { surface: "asrs" };
    expect(conversationMatchesSurface(metadata, "asrs")).toBe(true);
