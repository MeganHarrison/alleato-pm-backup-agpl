...
import {
  ...
  type AssistantTurnSourceKind,
  type AssistantTurnSourceReceipt,
  type AssistantTurnSourceReceiptStatus,
...
type AssistantTurnDatabaseClient = Pick<
...
function sourceReceipts(value: Json): readonly AssistantTurnSourceReceipt[] {
  ...
  return value.flatMap((item) => {
    ...
    const { id, kind, message, required, status } = item;
    if (
      typeof id !== "string" ||
      !isSourceKind(kind) ||
...
function isSourceKind(value: Json | undefined): value is AssistantTurnSourceKind {
  return value === "tool" || value === "source" || value === "artifact";
}

function isSourceStatus(
  value: Json | undefined,
): value is AssistantTurnSourceReceiptStatus {
  return (
    value === "pending" ||
    value === "succeeded" ||
    value === "warning" ||
...
export function createSupabaseAssistantTurnRepository(
  supabase: AssistantTurnDatabaseClient,
