 ...
 *
 * Dedicated chat products use this seam to persist the same row shape
 * (`session_id`, `user_id`, `role`, `content`, `metadata`) with one failure
 * contract.
 *
...
// Shared by server-owned assistant surfaces such as ASRS.
...
export function createChatHistoryWriter(
  ...
  function mergeLifecycleMetadata(
    metadata: ChatHistoryRecord["metadata"],
  ): Json | undefined {
    const hasLifecycleMetadata =
      ctx.assistantTurnId !== undefined || ctx.clientMessageId !== undefined;

    if (!hasLifecycleMetadata) {
      return metadata as Json | undefined;
    }
    ...
    return {
      ...existingMetadata,
      ...(ctx.assistantTurnId !== undefined
        ? { assistant_turn_id: ctx.assistantTurnId }
        : {}),
      ...(ctx.clientMessageId !== undefined
  ...
  async function persistRecord(
    ...
    const metadata = mergeLifecycleMetadata(record.metadata);
