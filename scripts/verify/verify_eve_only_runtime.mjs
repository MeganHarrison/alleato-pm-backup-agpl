...
const requireLegacyRetirement = process.argv.includes(
  "--require-legacy-retirement",
);
...
if (requireLegacyRetirement) {
  ...
  for (const { file, value } of forbiddenLegacyReferences) {
    ...
    if (
      ...
      failures.push(
        `Forbidden legacy assistant reference "${value}" is present in ${file}`,
      );
    }
  }
}
...
console.log(
  requireLegacyRetirement
    ? "Eve-only runtime guardrail passed: canonical runtime present, retired owners absent, and registry scaffolding removed."
    : "Eve transplant guardrail passed: canonical runtime present and registry scaffolding removed. Run with --require-legacy-retirement during AAI-1307 cutover.",
);
