-- ALL-52: convert leads only to compatible active CRM accounts.
begin;
...
begin
  ...
  if not exists (
    ...
    where profile.company_id = p_company_id
      and profile.owner_person_id = v_lead.owner_person_id
      and profile.archived_at is null
  ) then
    raise exception using
      errcode = '23514',
      message = 'Choose an active CRM account owned by the same relationship owner.';
  end if;
