-- Calendar rows are a governed projection, not a client-writable resource.
-- The earlier FOR ALL table policies allowed a project member to bypass the
-- validation and atomic replacement contract in replace_project_schedule_calendar.

begin;

revoke all on table public.project_schedule_calendars from anon, authenticated;
revoke all on table public.project_schedule_calendar_exceptions from anon, authenticated;

grant select on table public.project_schedule_calendars to authenticated;
grant select on table public.project_schedule_calendar_exceptions to authenticated;

drop policy if exists project_schedule_calendars_authenticated_access
  on public.project_schedule_calendars;
drop policy if exists project_schedule_calendar_exceptions_authenticated_access
  on public.project_schedule_calendar_exceptions;

create policy project_schedule_calendars_authenticated_read
  on public.project_schedule_calendars for select to authenticated
  using (public.current_is_app_admin() or public.current_is_project_member(project_id));

create policy project_schedule_calendar_exceptions_authenticated_read
  on public.project_schedule_calendar_exceptions for select to authenticated
  using (public.current_is_app_admin() or public.current_is_project_member(project_id));
