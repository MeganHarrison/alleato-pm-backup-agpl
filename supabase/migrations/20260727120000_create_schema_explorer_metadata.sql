-- Admin-only schema metadata for the internal Database Inventory surface.
-- It returns catalog metadata only: no table rows, credentials, or auth/private schemas.
  ...
  table_metadata as (
    select
      ...
      coalesce((
        select jsonb_agg(
          jsonb_build_object(
            ...
            'isPrimaryKey', pk_columns.attnum is not null
          )
          order by a.attnum
        )
      ...
      coalesce((
        ...
        join pg_catalog.pg_attribute primary_key
          on primary_key.attrelid = pk.conrelid
         and primary_key.attnum = primary_key_column.attnum
        where pk.conrelid = t.oid
          and pk.contype = 'p'
      ...
      coalesce((
        select jsonb_agg(
          jsonb_build_object(
            ...
            'referencedSchema', foreign_key.referenced_schema,
            'referencedTable', foreign_key.referenced_table,
            'referencedColumns', foreign_key.referenced_columns
          )
        ...
        from (
          select
            fk.conname as name,
            jsonb_agg(source_column.attname order by key_column.ordinality) as columns,
            target_namespace.nspname as referenced_schema,
            target_table.relname as referenced_table,
            jsonb_agg(target_column.attname order by key_column.ordinality) as referenced_columns
          ...
          join lateral unnest(fk.confkey) with ordinality as referenced_key_column(attnum, ordinality)
            on referenced_key_column.ordinality = key_column.ordinality
          join pg_catalog.pg_attribute target_column
            on target_column.attrelid = fk.confrelid
           and target_column.attnum = referenced_key_column.attnum
...
-- Functions are executable by PUBLIC by default. This catalog read must stay
-- behind the Next.js server route and its Admin Dashboard authorization gate.
