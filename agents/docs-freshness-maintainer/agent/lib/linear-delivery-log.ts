...
export function readLinearAgentSessionIdFromContinuationToken(
  token: string,
): string | null {
  if (!token.startsWith(LINEAR_SESSION_TOKEN_PREFIX)) return null;
  const id = token.slice(LINEAR_SESSION_TOKEN_PREFIX.length).trim();
  return id || null;
}

export function logLinearDeliveryEvent(
  event: string,
