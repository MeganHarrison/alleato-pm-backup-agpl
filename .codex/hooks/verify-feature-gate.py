...
FOUR gates, all fail-closed (exit 2 with instructions):

GATE 0 — protected Alleato routes cannot be opened with a raw anonymous
agent-browser command. The repository owns an auth preflight that refreshes an
origin-correct storage state, clears the daemon that ignores `--state`, and
proves the target route did not redirect to login. A raw browser launch skips
all of that work and is the recurring source of false "auth blocked" reports.
...
def gate_0_protected_browser_requires_auth_state(command: str) -> int:
    """Block anonymous agent-browser navigation to a protected Alleato route."""
    if "agent-browser" not in command or " open " not in f" {command} ":
        return 0

    protected_origin = "http://localhost:3000" in command or "https://projects.alleatogroup.com" in command
    auth_route = "/auth/login" in command or "/auth/" in command
    has_state = "--state" in command and "frontend/tests/.auth/user.json" in command
    uses_owned_verifier = "npm run verify:browser" in command or "prepare-authenticated-browser" in command

    if not protected_origin or auth_route or has_state or uses_owned_verifier:
        return 0

    print(
        "\n".join(
            [
                "BLOCKED by authenticated-browser readiness gate.",
                "",
                "A protected Alleato route was opened with raw agent-browser and no verified auth state.",
                "This is the recurring source of false 'unauthenticated locally' reports: raw agent-browser starts an anonymous daemon and does not load frontend/tests/.auth/user.json.",
                "",
                "Run the repository-owned preflight first:",
                "  npm run verify:browser-auth -- --base-url <origin> --route <route> --session <task>-auth-preflight",
                "",
                "Then every protected follow-up navigation must include:",
                "  agent-browser --state frontend/tests/.auth/user.json --session-name <task>-auth-preflight open <route>",
                "",
                "Only report authentication blocked if that preflight refreshes state, restarts the daemon, retries once, and still lands on /auth/login.",
            ]
        ),
        file=sys.stderr,
    )
...
def main() -> int:
    ...
    if "command" in data:
        ...
        auth_result = gate_0_protected_browser_requires_auth_state(command)
        if auth_result != 0:
            return auth_result
        return gate_1_browser_requires_criteria(command)
