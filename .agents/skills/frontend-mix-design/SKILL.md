...
2. The filename in `$ARGUMENTS` carries your run-name. Strip the directory and the `-plan.md` suffix. Example: `.Codex/artifacts/acme-saas-landing-plan.md` → run-name = `acme-saas-landing`. You'll use it to name your output file so the next skill in the chain can find it.
...
Write to `.Codex/artifacts/<run-name>-ui-summary.md`. Create the `.Codex/artifacts/` directory if it doesn't exist.
...
```
Wrote .Codex/artifacts/<run-name>-ui-summary.md

Next: back in Codex with Opus, invoke /frontend-mix-integrate with BOTH the plan path AND the ui-summary path.
```
