...
Both go to `.Codex/artifacts/` with the `<run-name>` prefix. Create the directory if it doesn't exist.

### Case A - All steps passed cleanly

Write `.Codex/artifacts/<run-name>-validation-summary.md` containing:
...
Write `.Codex/artifacts/<run-name>-validation-issues.md` containing, for each failure:
