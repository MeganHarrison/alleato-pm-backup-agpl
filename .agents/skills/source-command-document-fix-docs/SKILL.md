---
name: "source-command-document-fix-docs"
description: "Automatically fix documentation issues identified by audit"
---

# source-command-document-fix-docs

Use this skill when the user asks to run the migrated source command `document-fix-docs`.
  ...
  audit:
    ...
    steps:
      - name: Audit Documentation
        run: Codex audit-docs ./docs --format json > audit.json
      ...
      - name: Post PR Comment
        if: failure()
        run: |
          Codex fix-docs audit.json --dry-run --format markdown |
