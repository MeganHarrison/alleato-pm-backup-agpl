---
name: "source-command-workflow-implement-feature"
description: "Execute standardized Procore feature implementation workflow with auto-resume and parallel session support"
---

# source-command-workflow-implement-feature

Use this skill when the user asks to run the migrated source command `workflow-implement-feature`.
...
|-----------|-------------------------|
| `.Codex/research/{feature}.md` | RESEARCH done |
| `{feature}/TASKS.md` + `PLANS.md` | PLAN done |
| TASKS.md has `[x]` analysis markers | ANALYZE done |
| `.Codex/worker-done-{feature}-*.md` | IMPLEMENT in progress |
| `.Codex/tests-passing-{feature}-*.md` | TEST done |
...
Task({
  ...
  prompt: `Research {feature} feature:
    ...
    Write findings to: .Codex/research/{feature}.md`,
...
Task({
  ...
  prompt: `WORKER AGENT MODE
    ...
    Implement the task. Create .Codex/worker-done-{feature}-{task-id}.md when done.`,
...
Task({
  ...
  prompt: `TEST AGENT MODE
    ...
    Create .Codex/tests-passing-{feature}-{task-id}.md when done.`,
...
|---------------|---------------------|
| PLAN | `.Codex/research/{feature}.md` exists |
| ANALYZE | `TASKS.md` + `PLANS.md` exist |
| IMPLEMENT | TASKS.md has analysis markers |
| TEST | `.Codex/worker-done-{feature}-*.md` exists |
| VERIFY | `.Codex/tests-passing-{feature}-*.md` exists |
...
2. Each session checks `.Codex/locks/{feature}/` for claimed tasks
