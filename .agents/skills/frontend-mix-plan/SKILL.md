...
2. The filename in `$ARGUMENTS` carries your run-name. Strip the directory and the `-context.md` suffix. Example: `.Codex/artifacts/acme-saas-landing-context.md` → run-name = `acme-saas-landing`. You'll reuse it to name your output file so the next skill in the chain can find it.
...
Write the plan to `.Codex/artifacts/<run-name>-plan.md`. Create the `.Codex/artifacts/` directory if it doesn't exist.
...
```
Wrote .Codex/artifacts/<run-name>-plan.md
