...
> Gate** (`.Codex/rules/FORM-FK-VALIDATION-GATE.md`). The inline-edit select control on the detail
...
1. `cd frontend && npm run typecheck` (delegate the full run per AGENTS.md if a sub-agent is free).
...
| FK gate (editable dropdowns) | `.Codex/rules/FORM-FK-VALIDATION-GATE.md` |
