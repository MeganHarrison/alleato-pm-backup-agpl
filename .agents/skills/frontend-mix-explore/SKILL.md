...
Write the context dump to `.Codex/artifacts/<run-name>-context.md`. Create the `.Codex/artifacts/` directory if it doesn't exist.
...
```
Wrote .Codex/artifacts/<run-name>-context.md

Next: in Codex, switch the model to Opus, then invoke /frontend-mix-plan with the context.md path.
```
